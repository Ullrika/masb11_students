...
```{r}
#| message: false
source("kod/hypotes.R")
source("kod/styrkefkn.R")
```
...
Rutinen \texttt{styrkefkn} ritar upp styrkefunktionen, kommandot är \texttt{styrkefkn($\sigma$, $n$,   $\mu_0$, $\alpha$, H1-riktn, sant $\mu$)} om du vill rita upp funktionen och markera ett speciellt $\mu$-värde.  Använd alltså kommandot `styrka1(0.5, 7, 1, 0.05, '<', 0.8)`, vilket ger dig den tidigare figuren plus styrkan som en funktion av $\mu$.
:::
